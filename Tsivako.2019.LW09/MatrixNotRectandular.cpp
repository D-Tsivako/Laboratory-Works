...
void Matrix::displayNotRectandularMatrix(int** matrix, int rows, int* dimensions, char* fileName)
{
	...
	for (int i = 0; i < rows; i++)
	{
		displayArray(matrix[i], dimensions[i], fileName);
	}

	streamOut.close();
}

void Matrix::displayArray(int* array, int n, char* fileName)
{
...
void Matrix::displayNotRectandularMatrixAdd(int** matrix, int rows, int* dimensions, char* fileName)
{
	ofstream streamOut(fileName, ios::app);
	if (!streamOut.is_open())
	{
	...
	streamOut << "\nSorted matrix without repeating elements: \n";
	streamOut.close();
